...
minHeap *insert( minHeap *heap , char ch , int freq , Node *left , Node *right ) {
        ...
        minHeap *heapify( minHeap *heap , int i ) {    // i - index 
                if( heap->size <= 1 ) {
                        return heap;
                }
                ...
                if( left < heap->size && heap->array[left]->freq < heap->array[i]->freq )
                        min = left;
                if( right < heap->size && heap->array[right]->freq < heap->array[i]->freq )
                        min = right;
...
int main(int argc, char **argv)
{
        ...
        insert( heap , (char)121 , 2 , NULL , NULL );
        printf("--\n");
        print_heap(heap);

        insert( heap , 'o' , 1 , NULL , NULL );
        printf("--\n");
        print_heap(heap);
